...
class Configuration(commands.Cog):
    ...
    async def admin_role(
        ...
        role: Role = SlashOption(
            name="admin_role",
            description="The admin role. People with admin perm always have this authority.",
        ),
    ) -> None:
        """Set the admin role."""
        await interaction.client.db.set_admin_role(
            server_id=interaction.guild.id, role_id=role.id
        )
        await interaction.send(
            "I set the admin role to {}".format(role.mention), ephemeral=True
        )
    ...
    async def mod_role(
        ...
        role: Role = SlashOption(
            name="mod_role",
            description="The mod role. People with admin perm always have this authority.",
        ),
    ) -> None:
        """Set the mod role."""
        ...
        await interaction.send(
            "I set the mod role to {}".format(role.mention), ephemeral=True
        )

    @authority.subcommand()
    async def helper_role(
        self: Self,
        interaction: Interaction,
        role: Role = SlashOption(
            name="helper_role",
            description="The helper role. People with admin perm always have this authority.",
        ),
    ) -> None:
        """Set the helper role."""
        ...
        await interaction.send(
            "I set the helper role to {}".format(role.mention), ephemeral=True
        )
